    ...
    >
      <div className="container flex items-center justify-between">
        <a href="/" className="flex items-center gap-3 group">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 ${
              scrolled ? "bg-aog-green" : "bg-white/10 backdrop-blur-md"
            }`}
          >
            <Zap
              className={`w-6 h-6 transition-colors ${
                scrolled ? "text-white" : "text-aog-green"
              }`}
            />
          </div>
          <div
        ...
        <nav className="hidden md:flex items-center gap-8">
          <a
            href="#lead-form"
            className={`text-sm font-black uppercase tracking-widest transition-all hover:text-aog-green ${
          ...
          <a
            href="tel:+31612712804"
            onClick={() => trackContact({ method: "phone", source: "header" })}
            className={`text-sm font-black transition-all hover:scale-105 flex items-center gap-2 ${
              scrolled ? "text-aog-blue" : "text-white"
            }`}
          >
          ...
          <Button asChild size="lg" className="bg-aog-green hover:bg-aog-green-light text-white font-black rounded-2xl shadow-lg shadow-aog-green/20 h-12 px-6">
            <a href="#lead-form">Start de check</a>
          </Button>
        </nav>
        ...
        <button
          ...
          className={`md:hidden p-3 rounded-2xl transition-all ${
            scrolled ? "bg-slate-100 text-slate-900" : "bg-white/10 backdrop-blur-md text-white"
          }`}
          aria-label="Menu"
        >
      ...
      <div
        className={`md:hidden fixed inset-0 z-[-1] bg-white transition-all duration-500 ${
        ...
        <div className="container pt-32 pb-10 flex flex-col h-full">
          <nav className="flex flex-col gap-8 mb-auto">
            <a
              ...
              className="text-3xl font-black text-slate-900 flex items-center justify-between group"
            >
          ...
          <div className="mt-12 space-y-6">
            <a
              href="tel:+31612712804"
              onClick={() => trackContact({ method: "phone", source: "header_mobile" })}
              className="flex items-center gap-4 p-6 bg-slate-50 rounded-3xl group"
            >
