      ...
      <div className={`md:hidden fixed inset-0 z-[-1] bg-white transition-all duration-500 ${mobileOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full pointer-events-none"}`}>
        <div className="container pt-32 pb-10 flex flex-col h-full">
          <nav className="flex flex-col gap-8 mb-auto">
            ...
            <a href="#testimonials" onClick={() => setMobileOpen(false)} className="text-3xl font-black text-slate-900 flex items-center justify-between group">
              Klantervaringen <ArrowRight className="w-8 h-8 text-slate-200 group-hover:text-aog-green transition-colors" />
            </a>
          </nav>
          ...
          <div className="mt-12 space-y-6">
            ...
            <div className="flex items-center justify-center gap-2 text-slate-400">
              ...
              <span className="text-xs font-black uppercase tracking-widest">Onafhankelijk advies</span>
            </div>
          </div>
        </div>
      </div>
