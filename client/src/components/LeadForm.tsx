...
export default function LeadForm() {
  ...
  const calculatedSavings = useMemo(() => {
    ...
    return { savings: savingsWithBattery, extraCosts: totalExtraCost };
  }, [answers.panelCount]);
  ...
  // Steps: 1-6 quiz, 7 = form, 8 = disqualified (only)
  // NO spinner step anymore
  const nextStep = () => {
  ...
  const resetForm = () => {
    ...
    setAnswers({ homeType: "", hasPanels: "", panelCount: "", feedIn: "", usageMoment: "", futureUsage: "" });
    setLeadFields({ firstName: "", phone: "" });
  };
  ...
  const handleAutoAdvance = (key: keyof QuizAnswers, value: string) => {
    ...
    // Step 6 → 7 (form) direct, no spinner
  ...
  const handleLeadCapture = async () => {
    ...
    const firstName = leadFields.firstName.trim();
    ...
    try {
      ...
      if (sheetsAccepted || dbAccepted) {
        // ✅ Redirect naar /bedankt → Facebook Pixel CompleteRegistration vuurt hier
        setLocation(`/bedankt?naam=${encodeURIComponent(firstName)}&telefoon=${encodeURIComponent(phone)}`);
        return;
      }
      ...
      case 7:
        return (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Savings teaser — besparing zichtbaar als lokaas voor het formulier */}
            <div className="border-2 border-aog-green rounded-2xl p-5 mb-6 text-center bg-aog-green/5 relative overflow-hidden">
              ...
              <p className="text-sm font-bold text-slate-600 mb-1">🎉 Check afgerond! Jouw geschatte besparing:</p>
            ...
            <div className="border border-slate-200 rounded-2xl p-6 sm:p-8 bg-white shadow-sm">
              ...
              <p className="text-sm text-slate-500 mb-6">Onze specialist belt je op voor een gratis adviesgesprek aan huis.</p>
  ...
  return (
    <section id="lead-form" className="w-full scroll-mt-28">
      <div className={`rounded-[32px] sm:rounded-[40px] bg-white border border-slate-200 shadow-xl relative overflow-hidden transition-all duration-500 p-6 sm:p-10 max-w-2xl mx-auto`}>
