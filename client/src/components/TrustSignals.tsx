  ...
  return (
    <section className="py-12 sm:py-16 bg-white border-y border-slate-100 overflow-hidden">
      <div className="container mb-10">
        ...
        {/* Media Logos Grid — pointer-events-none prevents dead clicks (Clarity fix) */}
        <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-8 opacity-60 grayscale pointer-events-none select-none">
          {MEDIA_LOGOS.map((m) => (
            <div key={m.name} className="h-8 sm:h-10 flex items-center">
              <span className="text-xl sm:text-2xl font-black text-slate-400 tracking-tighter">
                {m.name}
              </span>
            </div>
          ))}
      ...
      <div className="relative mt-12 py-6 bg-slate-50 border-y border-slate-100">
        <div className="flex animate-[ticker_40s_linear_infinite] w-max gap-8 will-change-transform">
          {[...NEWS_ITEMS, ...NEWS_ITEMS].map((item, i) => (
            <div 
              key={i} 
              className="flex items-center gap-4 bg-white rounded-2xl px-6 py-4 border border-border shadow-sm"
            >
